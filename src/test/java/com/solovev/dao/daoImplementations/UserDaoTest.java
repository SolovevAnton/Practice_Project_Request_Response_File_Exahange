...
public class UserDaoTest {
    ...
    public class getByParamTests {
        ...
        public void getObjectsByParamSuccess() {
            String commonName = "third";
            DAO<User> userDAO = new UserDao();
            List<User> usersWithSameName = USERS.stream().filter(u-> u.getName().equals(commonName)).toList();

            assertEquals(USERS, userDAO.getObjectsByParam(Map.of()));
            assertEquals(usersWithSameName, userDAO.getObjectsByParam(Map.of("name", commonName)));
        }
